...
class CodeTemplateTestSuite:
    def __init__(self):
        ...
        self.node_test_script = Path(__file__).parent.parent / "front" / "test_generator.js"
...
'''
    ...
    def run_tests(self):
        ...
        try:
            ...
            tasks = session.query(ContentBlock).filter(
                and_(
            ...
            for i, task in enumerate(js_tasks, 1):
                ...
                print(f"Категория: {task.file.mainCategory if task.file else 'Unknown'}")
                ...
                self.results['statistics']['languages'][lang] = self.results['statistics']['languages'].get(lang, 0) + 1
                
                # Обновляем статистику по категориям
                category = task.file.mainCategory if task.file else 'unknown'
                self.results['statistics']['categories'][category] = self.results['statistics']['categories'].get(category, 0) + 1
...
"""
        ...
        if self.results['issues']:
            ...
            for problem, count in sorted(problem_types.items(), key=lambda x: x[1], reverse=True):
...
- **Основные проблемы:** {', '.join(list(set([issue for issue_data in self.results['issues'] for issue in issue_data['issues']]))[:3]) if self.results['issues'] else 'Не обнаружены'}
