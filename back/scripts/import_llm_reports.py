            ...
            try:
                ...
                if record:
                    ...
                    if self.imported_count % 10 == 0:
                        print(f"SUCCESS: Обработано {self.imported_count}/{len(md_files)}")
...
def generate_analytics_for_llm_reports(session: Session):
    ...
    llm_companies_stats = session.query(
        InterviewRecord.company_name,
        func.count(InterviewRecord.id).label('total')
    ).filter(InterviewRecord.source_type == 'llm_report').group_by(InterviewRecord.company_name).all()
