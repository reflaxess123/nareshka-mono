...
def import_data():
    ...
    try:
        # Путь к данным
        data_dir = Path(__file__).parent.parent.parent / "sobes-data" / "analysis" / "outputs_api_ready"
        ...
        print("\n2. Импорт кластеров...")
        with open(data_dir / "clusters.json", 'r', encoding='utf-8') as f:
        ...
        for i in range(0, len(questions_df), batch_size):
            ...
            for _, row in batch.iterrows():
                ...
                if not exists:
                    category_id = None
                    if pd.notna(row.get('final_category')) and row['final_category'] != 'Нет кластера':
                    ...
                    session.execute(
                        ...
                        {
                            'id': row['id'],
                            'question_text': row['question_text'][:2000],  # ограничиваем длину
                            'company': row.get('company') if pd.notna(row.get('company')) else None,
                            'cluster_id': cluster_id,
                            'category_id': category_id,
                            'topic_name': row.get('topic_name') if pd.notna(row.get('topic_name')) else None,
                            'canonical_question': row.get('canonical_question')[:2000] if pd.notna(row.get('canonical_question')) else None
                        }
                    )
                    imported += 1
                else:
            ...
            # Коммитим батчами
            session.commit()
            print(f"   Обработано: {min(i+batch_size, len(questions_df))}/{len(questions_df)}")
        ...
        print(f"   Категоризировано: {categorized} ({categorized/total_questions*100:.1f}%)")
