...
def populate_cluster_positions():
    ...
    try:
        ...
        for category_id, category_clusters in clusters_by_category.items():
            logger.info(
                f"Processing {len(category_clusters)} clusters for category {category_id}"
            )

            # Get category position for reference
            category = (
                session.query(InterviewCategory).filter_by(id=category_id).first()
            )
            if not category:
                continue
            ...
            positions = calculator.calculate_cluster_positions(
                cluster_data, use_tsne=False
            )
            ...
            logger.info(
                f"  ✅ Updated {len(positions)} clusters for category {category_id}"
            )

        session.commit()
        logger.info("✅ Successfully updated all cluster positions")
...
def populate_question_satellites():
    ...
    try:
        ...
        questions = (
            ...
            .filter(InterviewQuestion.cluster_id.isnot(None))
            .limit(1000)
            .all()
        )
...
def populate_cluster_connections():
    ...
    try:
        ...
        logger.info("✅ Created cluster connections")
...
def verify_population():
    ...
    try:
        ...
        populated_categories = (
            session.query(InterviewCategory)
            .filter(InterviewCategory.galaxy_x.isnot(None))
            .count()
        )
        ...
        populated_clusters = (
            session.query(InterviewCluster)
            .filter(InterviewCluster.star_x.isnot(None))
            .count()
        )
        ...
        populated_questions = (
            session.query(InterviewQuestion)
            .filter(InterviewQuestion.satellite_orbit_angle.isnot(None))
            .count()
        )
        ...
        logger.info(
            f"  Categories: {populated_categories}/{total_categories} have coordinates"
        )
        logger.info(
            f"  Clusters: {populated_clusters}/{total_clusters} have coordinates"
        )
        logger.info(
            f"  Questions: {populated_questions}/{total_questions} have satellite positions"
        )
        ...
        return {
            "categories": {
                ...
                "populated": populated_categories,
            },
