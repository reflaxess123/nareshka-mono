...
def create_unique_csv():
    ...
    input_file = Path(
        "../sobes-data/analysis/outputs_bertopic_final/all_questions_with_categories.csv"
    )
        ...
        new_data.append(
            {
                ...
                if row["final_category"] != "Нет кластера"
                else None,
                "topic_name": row["topic_name"]
                if row["topic_name"] != "Нет темы"
                else None,
                "canonical_question": row["canonical_question"]
                if pd.notna(row["canonical_question"])
                else None,
            }
        )
    ...
    for i in range(3):
        row = new_df.iloc[i]
        print(
            f"   {row['id']} (было: {row['original_question_id']}) -> {row['question_text'][:50]}..."
        )
...
def clear_and_reimport_data(csv_file):
    ...
    try:
        ...
        deleted_count = session.execute(
            text('DELETE FROM "InterviewQuestion"')
        ).rowcount
                ...
                try:
                    session.execute(
                        ...
                        """),
                        {
                            ...
                            "category_id": row["final_category"]
                            .replace(" ", "_")
                            .lower()
                            if pd.notna(row["final_category"])
                            else None,
                            "topic_name": row["topic_name"]
                            if pd.notna(row["topic_name"])
                            else None,
                            "canonical_question": row["canonical_question"][:2000]
                            if pd.notna(row["canonical_question"])
                            else None,
                        },
                    )
...
def verify_migration():
    ...
    try:
        ...
        positions_check = session.execute(
            text("""
        ...
        for pos in positions_check:
            print(
                f"   Позиция {pos.original_question_id}: {pos.count} вопросов из разных интервью"
            )
