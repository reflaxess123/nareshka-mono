...
class GracefulDegradation:
    ...
    @staticmethod
    def handle_redis_error(operation: str, error: Exception, default_value: Any = None) -> Any:
        """Обработка Redis ошибок с graceful degradation"""
        logger.error(f"Redis error in {operation}: {error}", extra={
            "operation": operation,
    ...
    @staticmethod
    def handle_external_api_error(service: str, error: Exception, default_value: Any = None) -> Any:
        """Обработка ошибок внешних API с graceful degradation"""
        logger.error(f"External API error ({service}): {error}", extra={
            "service": service,
            "error_type": type(error).__name__,
            "error_message": str(error)
        })
