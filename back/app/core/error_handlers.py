...
async def sqlalchemy_exception_handler(
    ...
    return JSONResponse(
        status_code=app_exception.http_status, content=app_exception.to_dict()
    )


async def redis_exception_handler(
    request: Request, exc: redis.RedisError
) -> JSONResponse:
...
async def validation_exception_handler(
    ...
    return JSONResponse(
        status_code=validation_exc.http_status, content=validation_exc.to_dict()
    )
...
async def http_exception_handler(
    ...
    return JSONResponse(
        status_code=exc.status_code,
        content={
            "error": {"code": error_code.value, "message": exc.detail, "details": {}}
        },
    )
