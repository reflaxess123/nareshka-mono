...
class ContentFile(Base):
    ...
    webdavPath = Column(
        String, unique=True, nullable=False, comment="Путь к файлу в WebDAV"
    )
    mainCategory = Column(
        String, nullable=False, comment="Основная категория (например, JavaScript)"
    )
    subCategory = Column(
        String, nullable=False, comment="Подкатегория (например, Arrays)"
    )
    lastFileHash = Column(
        String, comment="Хэш последней версии файла для отслеживания изменений"
    )
...
class ContentBlock(Base):
    ...
    isCodeFoldable = Column(
        Boolean, default=False, nullable=False, comment="Можно ли свернуть код"
    )
    codeFoldTitle = Column(String, comment="Заголовок для свернутого кода")

    # Extracted metadata
    extractedUrls: List[str] = Column(
        ARRAY(String), default=[], nullable=False, comment="Извлеченные из контента URL"
    )
