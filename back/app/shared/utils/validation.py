...
class FieldValidator:
    ...
    def length(
        ...
        if self.value:
            result = validate_string_length(
                self.value, min_length, max_length, self.field_name
            )
    ...
    def custom(
        self, validator_func: Callable[[Any], bool], error_message: str
    ) -> "FieldValidator":
...
def validate_user_registration_data(data: Dict[str, Any]) -> ValidationResult:
    ...
    # Validate username
    username_result = (
        validate_field("username", data.get("username"))
        .required()
        .length(3, 20)
        .custom(
    ...
    # Validate email
    email_result = (
        validate_field("email", data.get("email")).required().email().get_result()
    )
...
def log_validation_error(
    field: str, value: Any, error: str, user_id: Optional[str] = None
):
