...
class PaginationHelper:
    ...
    @staticmethod
    def paginate_query(query: SQLQuery, pagination: PaginationParams) -> tuple[List[Any], int]:
        """
        ...
        logger.debug("Query paginated", extra={
            "total": total,
            "page": pagination.page,
            "limit": pagination.limit,
    ...
    def paginate_list(items: List[T], pagination: PaginationParams) -> tuple[List[T], int]:
        ...
        logger.debug("List paginated", extra={
            "total": total,
            "page": pagination.page,
            "limit": pagination.limit,
    ...
    @staticmethod
    def create_response(items: List[T], total: int, pagination: PaginationParams) -> PaginatedResponse[T]:
        """Создать пагинированный ответ"""
...
class PaginatedQueryBuilder:
    ...
    def add_search(self, search_fields: List[str], search_term: str):
        ...
        if search_term:
            ...
            for field in search_fields:
                ...
                search_conditions.append(getattr(self.query.column_descriptions[0]['entity'], field).ilike(f"%{search_term}%"))
    ...
    def build(self, pagination: PaginationParams) -> PaginatedResponse:
        ...
        for field, (value, operator) in self.filters.items():
            ...
            if operator == "eq":
                self.query = self.query.filter(getattr(self.query.column_descriptions[0]['entity'], field) == value)
            elif operator == "in":
                self.query = self.query.filter(getattr(self.query.column_descriptions[0]['entity'], field).in_(value))
...
def create_sorting_dependency(default_sort_by: str = "id", allowed_fields: Optional[List[str]] = None):
    ...
    def get_sorting_params(
        ...
        if allowed_fields and sort_by not in allowed_fields:
            raise ValueError(f"Sorting by '{sort_by}' is not allowed. Allowed fields: {allowed_fields}")
