...
class CodeEditorRepository(CodeEditorRepositoryInterface):
    ...
    async def get_user_executions(self, user_id: int, block_id: Optional[str] = None, limit: int = 20, offset: int = 0) -> List[CodeExecution]:
        ...
        try:
            ...
            logger.info(f"Найдено {len(executions)} выполнений для пользователя {user_id}")
            return executions
        ...
        try:
            ...
            dangerous_patterns = {
                ...
                CodeLanguage.JAVASCRIPT: [
                    "require", "import", "fetch", "XMLHttpRequest", "document",
                    "window", "global", "process", "Buffer", "eval",
                    "Function", "setTimeout", "setInterval",
                ],
                CodeLanguage.JAVA: [
                    "System.exit", "Runtime", "ProcessBuilder", "File",
                    "FileInputStream", "FileOutputStream", "Socket",
                    "ServerSocket", "Class.forName", "reflect",
                ],
            }
    ...
    async def execute_code_with_language(self, source_code: str, language: SupportedLanguage, stdin: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            execution_result = {
                ...
                'container_logs': container.logs().decode('utf-8')[:1000],  # Ограничиваем размер
            }
