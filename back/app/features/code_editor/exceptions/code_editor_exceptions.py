...
class CodeEditorError(BaseAppException):
    ...
    def __init__(
        self, message: str = "Ошибка при работе с редактором кода", details: dict = None
    ):
        super().__init__(
            message=message,
...
class SolutionNotFoundError(ResourceNotFoundException):
    ...
    def __init__(
        self, solution_id: str = None, user_id: int = None, block_id: str = None
    ):
        if solution_id:
            super().__init__(
            ...
            super().__init__(resource_type="Solution", message="Решение")
