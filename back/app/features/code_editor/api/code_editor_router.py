...
async def get_execution_result(
    ...
    try:
        user_id = user.id if user else None
        execution = await code_editor_service.get_execution_result(
...
async def get_user_executions(
    ...
    try:
        ...
        executions = await code_editor_service.get_user_executions(user.id, blockId)
...
async def validate_solution(
    ...
    try:
        ...
        logger.info(
            f"API: Валидация завершена: {validation_result.passedTests}/{validation_result.totalTests}"
        )
