...
def get_universe_graph(
    ...
    layout: Literal["force", "circular", "hierarchical"] = Query("force", description="Layout algorithm"),
):
    """
    ...
    total_interviews_query = select(func.count(distinct(InterviewQuestion.interview_id)))
    ...
    cluster_query = select(
        ...
        func.count(distinct(InterviewQuestion.interview_id)).label("interview_count"),
        func.round(
            func.count(distinct(InterviewQuestion.interview_id)) * 100.0 / total_interviews, 2
        ).label("interview_penetration"),
    ).select_from(InterviewCluster).join(
        InterviewCategory, InterviewCluster.category_id == InterviewCategory.id
    ).outerjoin(
    ...
    if min_penetration:
        filters.append(
            func.count(distinct(InterviewQuestion.interview_id)) * 100.0 / total_interviews >= min_penetration
        )
    ...
    for cluster in clusters:
        ...
        company_query = select(
            ...
            InterviewQuestion.cluster_id == cluster.id
        ).group_by(
            InterviewQuestion.company
        ).order_by(
            func.count().desc()
...
def get_cluster_details(
    ...
    if include_questions:
        question_query = select(
            ...
            InterviewQuestion.cluster_id == cluster_id
