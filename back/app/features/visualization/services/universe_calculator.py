...
class UniverseCalculator:
    ...
    def calculate_galaxy_positions(self, categories: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for i, category in enumerate(categories):
            ...
            question_ratio = category.get('questions_count', 0) / max(total_questions, 1)
    ...
    def calculate_cluster_positions(
        ...
        if embeddings is not None and embeddings.shape[0] == n_clusters and SKLEARN_AVAILABLE:
            ...
            if self.scaler:
                ...
                coords_3d = (coords_3d - coords_3d.mean(axis=0)) / (coords_3d.std(axis=0) + 1e-8) * 100
    ...
    def calculate_company_positions(
        ...
        for company in companies:
            ...
            if primary_cluster:
                ...
                planet_x = primary_cluster['star_x'] + orbit_distance * np.cos(orbit_angle)
                planet_y = primary_cluster['star_y'] + orbit_distance * np.sin(orbit_angle)
    ...
    def calculate_question_positions(
        ...
        for question in questions:
            ...
            if company_id and company_id in company_positions:
                ...
                sat_x = company_pos['x'] + sat_distance * np.sin(sat_phi) * np.cos(sat_angle)
                sat_y = company_pos['y'] + sat_distance * np.sin(sat_phi) * np.sin(sat_angle)
    ...
    def _estimate_difficulty(self, cluster: Dict[str, Any]) -> int:
        ...
        difficulty_keywords = {
            'advanced': 3, 'complex': 3, 'optimization': 2,
            'algorithm': 2, 'architecture': 3, 'design pattern': 2,
            'performance': 2, 'senior': 3, 'expert': 3
        }
    ...
    def _get_category_index(self, category_id: str) -> int:
        ...
        category_map = {
            'javascript_core': 0, 'react': 1, 'typescript': 2,
            'soft_skills': 3, 'сеть': 4, 'алгоритмы': 5,
            'верстка': 6, 'браузеры': 7, 'архитектура': 8,
            'инструменты': 9, 'производительность': 10,
            'тестирование': 11, 'другое': 12
        }
