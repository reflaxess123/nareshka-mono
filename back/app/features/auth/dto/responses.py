...
class UserResponse(BaseResponse):
    ...
    last_activity_date: Optional[datetime] = Field(
        None, description="Last activity date"
    )
...
class RegisterResponse(BaseResponse):
    ...
    tokens: Optional[TokenResponse] = Field(
        None, description="Authentication tokens (if auto-login)"
    )
    message: str = Field(
        default="Registration successful", description="Success message"
    )
...
class LogoutResponse(BaseResponse):
    ...
    logged_out_devices: int = Field(
        default=1, description="Number of devices logged out"
    )

    class Config:
        schema_extra = {
            "example": {"message": "Logout successful", "logged_out_devices": 1}
        }
...
class PasswordResetResponse(BaseResponse):
    ...
    message: str = Field(
        default="Password reset email sent", description="Success message"
    )
...
class PasswordResetConfirmResponse(BaseResponse):
    ...
    message: str = Field(
        default="Password reset successful", description="Success message"
    )
    user: UserResponse = Field(..., description="User information")
    tokens: Optional[TokenResponse] = Field(
        None, description="New tokens (if auto-login)"
    )
...
class ChangePasswordResponse(BaseResponse):
    ...
    logout_other_sessions: bool = Field(
        default=True, description="Whether other sessions were logged out"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "details": {"field": "email", "code": "invalid_credentials"},
            }
        }
