...
class AuthService:
    ...
    async def get_user_by_session(self, request) -> User:
        ...
        logger.info(
            "🔍 DEBUG: get_user_by_session called",
            extra={
        ...
        if not session_id:
            logger.info("🔍 DEBUG: No session_id cookie found")
            raise HTTPException(
        ...
        logger.info(
            ...
            extra={"session_id_prefix": session_id[:10] + "...", "user_id": user_id},
        )

        if not user_id:
            logger.info("🔍 DEBUG: Session expired or not found in Redis")
            raise HTTPException(
        ...
        if not user:
            logger.info(
                "🔍 DEBUG: User not found in database", extra={"user_id": user_id}
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="User not found"
            )
        ...
        logger.info(
            "🔍 DEBUG: User successfully retrieved from session",
