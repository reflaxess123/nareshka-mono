...
class ContentProgressUpdateRequest(BaseModel):
    ...
    time_spent_minutes: Optional[int] = Field(
        None, description="Потраченное время в минутах"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Дополнительные данные"
    )
...
class CategoryProgressUpdateRequest(BaseModel):
    ...
    def validate_completed_vs_total(cls, v, values):
        if "total_tasks" in values and v > values["total_tasks"]:
            raise ValueError(
                "Количество выполненных задач не может превышать общее количество"
            )
        return v
    ...
    def validate_attempted_vs_completed(cls, v, values):
        if "completed_tasks" in values and v < values["completed_tasks"]:
            raise ValueError(
                "Количество попыток не может быть меньше выполненных задач"
            )
        return v
...
class ProgressAnalyticsRequest(BaseModel):
    ...
    user_id: Optional[int] = Field(
        None, description="ID пользователя (None для общей статистики)"
    )
