...
from app.shared.utils.http import PaginationParams, ApiResponse
from app.shared.dependencies import get_db_manager as get_database_manager
from app.shared.database.base import DatabaseManager

# Импорты auth для проверки пользователей
from app.shared.dependencies import get_current_user_required, get_current_admin_session
...
async def update_category_progress(
    ...
    current_admin: User = Depends(get_current_admin_session),  # Только админ может обновлять прогресс по категориям
    progress_service: ProgressService = Depends(get_progress_service)
) -> CategoryProgressResponse:
...
async def get_user_category_progress(
    current_user: User = Depends(get_current_user_required),
    main_category: Optional[str] = Query(None, description="Фильтр по основной категории"),
    progress_service: ProgressService = Depends(get_progress_service)
