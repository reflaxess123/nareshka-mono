...
from app.shared.exceptions.base import ValidationError, NotFoundError, ConflictError
...
try:
    ...
    from app.features.task.dto.responses import TaskAttemptResponse, TaskSolutionResponse
except ImportError:
...
class ProgressService:
    ...
    @transactional
    async def create_task_attempt(self, request: TaskAttemptCreateRequest) -> TaskAttemptResponse:
        """Создание попытки решения задачи"""
        logger.info("Creating task attempt", extra={
        ...
        try:
            ...
            logger.info("Task attempt created successfully", extra={
            ...
            logger.error("Failed to create task attempt", extra={
                "user_id": request.user_id,
                "task_id": request.task_id,
                "error": str(e)
            })
    ...
    async def get_attempt_history(self, user_id: int, task_id: str) -> AttemptHistoryResponse:
        ...
        best_time = min([a.execution_time_ms for a in successful_attempts if a.execution_time_ms]) if successful_attempts else None
        first_success_attempt = successful_attempts[0].attempt_number if successful_attempts else None
        ...
        return AttemptHistoryResponse(
            ...
            solution=TaskSolutionResponse.model_validate(solution) if solution else None
        )
    
    # === Task Solutions ===
    
    @transactional
    async def create_task_solution(self, request: TaskSolutionCreateRequest) -> TaskSolutionResponse:
        """Создание решения задачи"""
        logger.info("Creating task solution", extra={
            "user_id": request.user_id,
            "task_id": request.task_id
        })
    ...
    async def get_user_solutions(self, user_id: int, limit: int = 50) -> List[TaskSolutionResponse]:
    ...
    async def get_task_solution(self, user_id: int, task_id: str) -> Optional[TaskSolutionResponse]:
    ...
    @transactional
    async def update_content_progress(self, request: ContentProgressUpdateRequest) -> ContentProgressResponse:
    ...
    async def get_user_content_progress(
        ...
        return [ContentProgressResponse.model_validate(progress) for progress in progress_list]
    
    # === Category Progress ===
    
    @transactional
    async def update_category_progress(self, request: CategoryProgressUpdateRequest) -> CategoryProgressResponse:
    ...
    async def get_category_progress(
        ...
        return [CategoryProgressResponse.model_validate(progress) for progress in progress_list]
    
    # === User Progress Summary ===
    
    async def get_user_progress_summary(self, user_id: int) -> UserProgressSummaryResponse:
    ...
    async def get_user_detailed_progress(self, user_id: int) -> UserDetailedProgressResponse:
    ...
    async def get_progress_analytics(self, request: ProgressAnalyticsRequest) -> ProgressAnalyticsResponse:
    ...
    def _group_categories(
        ...
        for progress in category_progress:
            ...
            if (not grouped[main_cat]['last_activity'] or 
                (progress.last_activity and progress.last_activity > grouped[main_cat]['last_activity'])):
        ...
        for main_cat, data in grouped.items():
            ...
            result.append(GroupedCategoryProgressResponse(
                main_category=main_cat,
