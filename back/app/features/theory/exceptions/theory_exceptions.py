...
class TheoryError(BaseAppException):
    ...
    def __init__(
        self, message: str = "Ошибка при работе с теорией", details: dict = None
    ):
        super().__init__(
            message=message, error_code="THEORY_ERROR", status_code=500, details=details
        )
...
class InvalidProgressActionError(TheoryError):
    ...
    def __init__(self, action: str):
        super().__init__(
            f"Недопустимое действие: {action}. Разрешены: increment, decrement"
        )
...
class InvalidReviewRatingError(TheoryError):
    ...
    def __init__(self, rating: str):
        super().__init__(
            f"Недопустимая оценка: {rating}. Разрешены: again, hard, good, easy"
        )
