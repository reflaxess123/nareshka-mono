...
class TaskService:
    ...
    async def get_tasks(
        ...
        logger.info(
            f"Получение заданий: page={page}, limit={limit}, categories={main_categories}"
        )
        ...
        try:
            ...
            for task in tasks:
                task_response = TaskResponse(
                    ...
                    currentUserSolvedCount=getattr(
                        task, "current_user_solved_count", 0
                    ),
    ...
    async def get_task_companies(
        ...
        logger.info(
            f"Получение компаний: categories={main_categories}, subcategories={sub_categories}"
        )
    ...
    async def create_task_attempt(
        ...
        logger.info(
            f"Создание попытки пользователя {user_id} для блока {request.blockId}"
        )
        ...
        try:
            ...
            if not request.sourceCode.strip():
                raise TaskValidationError(
                    "sourceCode", "Исходный код не может быть пустым"
                )

            if not request.language.strip():
                raise TaskValidationError(
                    "language", "Язык программирования обязателен"
                )
    ...
    async def create_task_solution(
        ...
        logger.info(
            f"Создание решения пользователя {user_id} для блока {request.blockId}"
        )
        ...
        try:
            ...
            if not request.finalCode.strip():
                raise TaskValidationError(
                    "finalCode", "Финальный код не может быть пустым"
                )
            ...
            if request.totalAttempts <= 0:
                raise TaskValidationError(
                    "totalAttempts", "Количество попыток должно быть больше 0"
                )
    ...
    async def get_user_task_attempts(
        ...
        try:
            attempts = await self.task_repository.get_user_task_attempts(
                user_id, block_id
            )

            attempt_responses = [
                TaskAttemptResponse.from_orm(attempt) for attempt in attempts
            ]
    ...
    async def get_user_task_solutions(
        ...
        try:
            solutions = await self.task_repository.get_user_task_solutions(
                user_id, block_id
            )

            solution_responses = [
                TaskSolutionResponse.from_orm(solution) for solution in solutions
            ]
