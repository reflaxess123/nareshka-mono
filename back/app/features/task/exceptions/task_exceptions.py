...
class TaskError(BaseAppException):
    ...
    def __init__(
        self, message: str = "Ошибка при работе с заданиями", details: dict = None
    ):
        super().__init__(
            message=message,
...
class TaskNotFoundError(ResourceNotFoundException):
    ...
    def __init__(self, task_id: str):
        super().__init__(resource_type="Task", resource_id=task_id)
...
class TaskValidationError(ValidationException):
    ...
    def __init__(self, field: str, message: str = None):
        full_message = f"Ошибка валидации поля '{field}'" + (
            f": {message}" if message else ""
        )
        super().__init__(message=full_message, field=field)
...
class TaskCodeExecutionError(CodeExecutionException):
    ...
    def __init__(
        self, message: str = "Ошибка выполнения кода", execution_details: str = None
    ):
        super().__init__(
            message=message,
            details={"execution_details": execution_details}
