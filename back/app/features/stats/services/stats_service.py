...
class StatsService:
    ...
    async def get_roadmap_stats(self, user_id: int) -> RoadmapStatsResponse:
        ...
        try:
            ...
            return RoadmapStatsResponse(categories=stats_data["categories"])
    ...
    async def get_stats_health(self) -> StatsHealthResponse:
        ...
        return StatsHealthResponse(status="healthy", module="stats")
    ...
    async def get_category_comparison(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for category_name in comparison:
                content_perc = (
                    comparison[category_name].get("content", {}).get("percentage", 0)
                )
                theory_perc = (
                    comparison[category_name].get("theory", {}).get("percentage", 0)
                )
