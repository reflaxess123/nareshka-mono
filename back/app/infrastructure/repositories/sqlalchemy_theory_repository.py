...
from app.infrastructure.models.theory_models import (
    ...
    UserTheoryProgress as SQLUserTheoryProgress,
)
...
class SQLAlchemyTheoryRepository(TheoryRepository):
    ...
    async def get_theory_cards(
        ...
        if sub_category:
            query = query.filter(
                func.lower(SQLTheoryCard.subCategory) == func.lower(sub_category)
            )
        ...
        if only_unstudied and user_id:
            studied_card_ids = (
                self.session.query(SQLUserTheoryProgress.cardId)
                .filter(
    ...
    async def get_theory_categories(self) -> List[Dict[str, Any]]:
        ...
        categories_data = (
            ...
            .group_by(SQLTheoryCard.category, SQLTheoryCard.subCategory)
            .all()
        )
        ...
        for category, sub_category, count in categories_data:
            ...
            if sub_category:
                grouped_categories[category]["subCategories"].append(
                    {"name": sub_category, "cardCount": count}
                )
    ...
    async def get_theory_subcategories(self, category: str) -> List[str]:
        ...
        return [
            row[0]
            for row in self.session.query(SQLTheoryCard.subCategory)
            .filter(
                func.lower(SQLTheoryCard.category) == func.lower(category),
    ...
    async def get_due_theory_cards(
        ...
        if len(due_infra_cards) < limit:
            ...
            new_cards_subquery = (
                self.session.query(SQLUserTheoryProgress.cardId)
                .filter(SQLUserTheoryProgress.userId == user_id)
                .subquery()
            )

            new_infra_cards = (
                self.session.query(SQLTheoryCard)
                .filter(~SQLTheoryCard.id.in_(new_cards_subquery))
    ...
    async def get_theory_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        total_cards = (
            self.session.query(func.count(SQLTheoryCard.id))
            .filter(
                ~SQLTheoryCard.category.ilike("%QUIZ%"),
                ~SQLTheoryCard.category.ilike("%ПРАКТИКА%"),
            )
        ...
        # Изученные карточки
        studied_cards = (
            self.session.query(func.count(SQLUserTheoryProgress.id))
            .filter(
                SQLUserTheoryProgress.userId == user_id,
                SQLUserTheoryProgress.solvedCount > 0,
            )
        ...
        category_stats = (
            ...
            .group_by(SQLTheoryCard.category)
            .all()
        )
