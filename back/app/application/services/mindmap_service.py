...
from app.domain.entities.mindmap_types import (
    MindMapEdge,
    MindMapNode,
    MindMapResponse,
    TaskDetail,
...
class MindMapService:
    ...
    def generate_mindmap(
        ...
        for i, topic in enumerate(active_topics):
            ...
            edge = MindMapEdge(
                ...
                style={"stroke": topic.color, "strokeWidth": 3, "strokeOpacity": 0.7},
                animated=True,
            )
            edges.append(edge)
        ...
        return MindMapResponse(
            ...
            applied_filters={"difficulty": difficulty_filter, "topic": topic_filter},
    ...
    def get_available_technologies(self) -> Dict[str, Any]:
        ...
        return {"technologies": technologies, "configs": technology_configs}
    ...
    def get_topic_with_tasks(
        ...
        if not topic:
            raise ValueError(
                f"Topic '{topic_key}' not found for technology '{technology}'"
            )

        # Получаем задачи по теме
        tasks = self.mindmap_repository.get_topic_tasks(
            topic, user_id, difficulty_filter
        )
        ...
        return TopicWithTasks(topic=topic, tasks=tasks, stats=stats)
    ...
    def get_health_status(self) -> Dict[str, str]:
        ...
        return {"status": "healthy", "module": "mindmap"}
