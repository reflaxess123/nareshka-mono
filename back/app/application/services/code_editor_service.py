...
class CodeEditorService:
    ...
    async def execute_code(
        self, request: CodeExecutionRequestDTO, user_id: Optional[int] = None
    ) -> CodeExecutionResponseDTO:
        ...
        if not language:
            raise ValueError(
                f"Language {request.language} is not supported or disabled"
            )
        ...
        # Сохраняем в БД
        created_execution = await self.code_editor_repository.create_code_execution(
            execution
        )
    ...
    async def get_execution_result(
        ...
        execution = await self.code_editor_repository.get_execution_by_id_and_user(
    ...
    async def save_solution(
        self, solution_data: UserCodeSolutionCreateDTO, user_id: int
    ) -> UserCodeSolutionResponseDTO:
        ...
        existing_solution = (
            await self.code_editor_repository.get_solution_by_user_and_block(
        ...
        if existing_solution:
            ...
            updated_solution = await self.code_editor_repository.create_user_solution(
                new_solution
            )
    ...
    ) -> List[UserCodeSolutionResponseDTO]:
    ...
    async def update_solution(
        self, solution_id: str, user_id: int, update_data: UserCodeSolutionUpdateDTO
    ) -> UserCodeSolutionResponseDTO:
        """Обновление решения пользователя"""

        solution = await self.code_editor_repository.get_solution_by_id_and_user(
            solution_id, user_id
        )
        if not solution:
    ...
    async def get_execution_stats(self, user_id: int) -> ExecutionStatsDTO:
        ...
        total_executions = (
            await self.code_editor_repository.get_execution_count_by_user(user_id)
        )
        successful_executions = (
            await self.code_editor_repository.get_successful_execution_count_by_user(
                user_id
            )
        )
        average_execution_time = (
            await self.code_editor_repository.get_average_execution_time(user_id)
        )
        language_stats = await self.code_editor_repository.get_language_stats_by_user(
            user_id
        )
    ...
    async def get_test_cases(
        ...
        if user_id:
            test_cases = (
                await self.code_editor_repository.get_public_test_cases_for_block(
                    block_id
                )
            )
        else:
    ...
    async def validate_solution(
        self, block_id: str, validation_request: ValidationRequestDTO, user_id: int
    ) -> ValidationResultDTO:
