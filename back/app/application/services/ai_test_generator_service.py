...
Return ONLY the JSON array, no additional text or markdown formatting.
    ...
    def _analyze_task(self, block: ContentBlock) -> Dict:
        ...
        return {
            ...
            "has_loops": any(keyword in code.lower() for keyword in ['for', 'while', 'loop']),
            "has_conditions": any(keyword in code.lower() for keyword in ['if', 'else', 'switch', 'case']),
            "has_functions": any(keyword in code.lower() for keyword in ['def', 'function', 'func'])
        }
    ...
    def _generate_with_patterns(
        ...
        if task_type in self.fallback_patterns:
            ...
            for i in range(min(request.count, len(base_patterns))):
                ...
                patterns.append(TestCasePattern(
    ...
    async def _create_test_cases(
        ...
        for i, pattern in enumerate(patterns):
            test_case = TestCase(
                ...
                generationPrompt=f"Generated by {provider}" if provider == "openai" else None,
