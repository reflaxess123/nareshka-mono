...
class TestCase(Base):
    ...
    # Метаданные
    isPublic = Column(Boolean, default=True, nullable=False)  # Показывать пользователю или скрытый
    ...
    __table_args__ = (
        ...
        Index("idx_testcase_active", "isActive"),
...
class TestValidationResult(Base):
    ...
    id = Column(String, primary_key=True)
    testCaseId = Column(String, ForeignKey("TestCase.id", ondelete="CASCADE"), nullable=False)
    attemptId = Column(String, ForeignKey("TaskAttempt.id", ondelete="CASCADE"), nullable=False)
    ...
    __table_args__ = (
        Index("idx_testvalidation_testcaseid", "testCaseId"),
        Index("idx_testvalidation_attemptid", "attemptId"),
    )
