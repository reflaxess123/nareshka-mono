...
class SQLAlchemyStatsRepository(StatsRepository):
    ...
    async def get_user_stats_overview(  # noqa: PLR0912, PLR0915
        ...
        for block in content_blocks:
            ...
            sub_category = (
                block.file.subCategory if block.file.subCategory else "General"
            )
        ...
        theory_progress = (
            self.db_session.query(UserTheoryProgress)
        ...
        content_percentage = (
            round((solved_content_blocks / total_content_blocks) * 100, 2)
            if total_content_blocks > 0
            else 0
        )
        theory_percentage = (
            round((reviewed_theory_cards / total_theory_cards) * 100, 2)
            if total_theory_cards > 0
            else 0
        )
    ...
    async def get_content_stats(self, user_id: int) -> ContentStats:
        ...
        blocks_with_progress = (
            self.db_session.query(ContentBlock, UserContentProgress)
            ...
            if sub_category not in categories[category].subCategories:
                categories[category].subCategories[sub_category] = (
                    SubCategoryContentStats(
                        total=0, solved=0, percentage=0, averageSolveCount=0, blocks=[]
                    )
                )
        ...
        for cat_data in categories.values():
            if cat_data.total > 0:
                ...
                cat_data.averageSolveCount = round(
            ...
            for sub_cat in cat_data.subCategories.values():
                if sub_cat.total > 0:
                    ...
                    sub_cat.averageSolveCount = round(
                        sub_solve_count / sub_cat.total, 2
                    )

        average_solve_count = (
            round(total_solve_count / total_blocks, 2) if total_blocks > 0 else 0
        )
    ...
    async def get_theory_stats(self, user_id: int) -> TheoryStats:
        ...
        cards_with_progress = (
            self.db_session.query(TheoryCard, UserTheoryProgress)
            .outerjoin(
                UserTheoryProgress,
        ...
        for card, progress in cards_with_progress:
            ...
            card_stats = TheoryCardStats(
                ...
                if len(card.questionBlock or "") > 100
        ...
        for cat_data in categories.values():
            if cat_data.total > 0:
                ...
                cat_data.averageReviewCount = round(
            ...
            for sub_cat in cat_data.subCategories.values():
                if sub_cat.total > 0:
                    ...
                    sub_cat.averageReviewCount = round(
                        sub_review_count / sub_cat.total, 2
                    )

        average_review_count = (
            round(total_review_count / total_cards, 2) if total_cards > 0 else 0
        )
    ...
    async def get_roadmap_stats(self, user_id: int) -> RoadmapStats:
        ...
        content_categories = (
            ...
            .distinct()
            .order_by(ContentFile.mainCategory.asc(), ContentFile.subCategory.asc())
            .all()
        )

        theory_categories = (
            self.db_session.query(TheoryCard.category, TheoryCard.subCategory)
            .distinct()
            .order_by(TheoryCard.category.asc(), TheoryCard.subCategory.asc())
            .all()
        )
        ...
        for category_name in sorted(all_categories):
            ...
            content_blocks = (
                ...
                .join(ContentFile)
                .filter(ContentFile.mainCategory == category_name)
                .all()
            )

            # Получаем статистику по теории для этой категории
            theory_cards = (
                self.db_session.query(TheoryCard)
                .filter(TheoryCard.category == category_name)
                .all()
            )
            ...
            content_completed = len(
                [
                    b
                    for b in content_blocks
                    if content_progress_dict.get(b.id)
                    and content_progress_dict[b.id].solvedCount > 0
                ]
            )
            theory_total = len(theory_cards)
            theory_completed = len(
                [
                    c
                    for c in theory_cards
                    if theory_progress_dict.get(c.id)
                    and theory_progress_dict[c.id].reviewCount > 0
                ]
            )

            content_progress_pct = (
                round((content_completed / content_total) * 100)
                if content_total > 0
                else 0
            )
            theory_progress_pct = (
                round((theory_completed / theory_total) * 100)
                if theory_total > 0
                else 0
            )
            ...
            overall_progress_pct = (
                round((total_completed_items / total_items) * 100)
                if total_items > 0
                else 0
            )
            ...
            category_data = CategoryRoadmapStats(
                ...
                contentStats=ItemStats(
                    total=content_total, completed=content_completed
                ),
                theoryStats=ItemStats(total=theory_total, completed=theory_completed),
    ...
    async def get_content_blocks_by_category(self, category: str) -> List[Any]:
        return (
            self.db_session.query(ContentBlock)
            .join(ContentFile)
            .filter(ContentFile.mainCategory == category)
            .all()
        )
    ...
    async def get_theory_cards_by_category(self, category: str) -> List[Any]:
        return (
            self.db_session.query(TheoryCard)
            .filter(TheoryCard.category == category)
            .all()
        )

    async def get_user_content_progress_for_blocks(
    ...
    async def get_user_theory_progress_for_cards(
        self, user_id: int, card_ids: List[str]
    ) -> List[Any]:
        return (
            self.db_session.query(UserTheoryProgress)
            .filter(
