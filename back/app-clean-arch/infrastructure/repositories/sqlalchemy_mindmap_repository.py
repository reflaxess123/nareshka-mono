...
class SqlAlchemyMindMapRepository(MindMapRepository):
        ...
        for block in content_blocks:
            ...
            if user_id:
                ...
                if user_progress:
                    ...
                    progress = {"solvedCount": 0, "isCompleted": False}
    ...
    def get_task_by_id(
        ...
        block = (
            self.session.query(ContentBlock).filter(ContentBlock.id == task_id).first()
        )
        if not block:
            return None
    ...
    def get_topic_stats(
        self, topic: Topic, user_id: Optional[int] = None
    ) -> Optional[TopicStats]:
        ...
        completed_tasks = sum(
            1 for task in tasks if task.progress and task.progress["isCompleted"]
        )
        progress_percentage = (
            (completed_tasks / total_tasks * 100) if total_tasks > 0 else 0
        )
