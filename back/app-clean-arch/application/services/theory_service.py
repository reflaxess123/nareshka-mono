...
from app.domain.entities.theory_types import UserTheoryProgress
...
class TheoryService:
        ...
        # Повторяемая карточка
            ...
            if quality >= 3:
                ...
                new_ease = max(
                    ...
                    + Decimal("0.1")
                    - (5 - quality)
                    * (Decimal("0.08") + (5 - quality) * Decimal("0.02")),
                )
                ...
                if quality == 3:
                    ...
                    new_interval = max(
                        1, int(current_interval * new_ease * Decimal("1.3"))
                    )
            else:
    ...
    async def get_theory_cards(
        ...
        if user_id:
            ...
            for card in cards:
                ...
                if progress:
                    card_response.progress = UserTheoryProgressResponse.from_orm(
                        progress
                    )
                else:
    ...
    async def get_theory_card_by_id(
        ...
        if not card:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Theory card not found"
            )
    ...
    async def update_theory_card_progress(
        ...
        return await self.theory_repository.create_or_update_user_progress(
            user_id=user_id, card_id=card_id, solvedCount=new_count
        )
    ...
    async def review_theory_card(
        ...
        return await self.theory_repository.create_or_update_user_progress(
            user_id=user_id, card_id=card_id, **update_data
        )
